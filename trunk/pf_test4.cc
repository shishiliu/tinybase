...
RC WriteFile(PF_Manager &pfm, char *fname)
{
   ...
   for (i = 0; i < PF_BUFFER_SIZE; i++) {
      ...
      //lss
      int zero = 0;
      memcpy(pData, (char *)&zero, sizeof(zero));	
      //lss 
...
//here, PF_BUFFER_SIZE == total numbre of pages
...
//lss
	char words = 'A';
	memcpy(pData, (char*)&words, sizeof(words));
//lss
      //  memcpy(pData + PF_PAGE_SIZE - sizeof(PageNum), &pageNum, sizeof(PageNum));
