...
BTNode::BTNode(AttrType attrType, int attrLength,
...
BTNode::~BTNode()
{
  // cerr << "Destructor for BTNode - page id " << pageRID << endl;
};

RC BTNode::ResetBTNode(PF_PageHandle& ph, const BTNode& rhs)
{
  order = (rhs.order);
...
// sets number of keys
// returns -1 on error
int BTNode::SetNumKeys(int newNumKeys)
{
...
// get/set pageRID
RID BTNode::GetPageRID() const
{
  return pageRID;
}
void BTNode::SetPageRID(const RID& r)
{
  pageRID = r;
}
...
RC BTNode::IsValid() const
{
  ...
  if(!ret)
  {
    std::cerr << "order was " << order << " numkeys was  " << numKeys << std::endl;
  }
  return ret ? 0 : IX_BADIXPAGE;
}


int BTNode::GetMaxKeys() const
{
  assert(IsValid() == 0);
  return order;
}
...
RC BTNode::GetKey(int pos, void* &key) const
{
  assert(IsValid() == 0);
...
int BTNode::CopyKey(int pos, void* toKey) const
{
  assert(IsValid() == 0);
...
// points to
// returns -1 on error
int BTNode::SetKey(int pos, const void* newkey)
{
  assert(IsValid() == 0);
  assert(pos >= 0 && pos < order);
...
int BTNode::Insert(const void* newkey, const RID & rid)
{
  assert(IsValid() == 0);
...
int BTNode::Remove(const void* newkey, int kpos)
{
  assert(IsValid() == 0);
  int pos = -1;
  if (kpos != -1) {
...
RID BTNode::FindAddrAtPosition(const void* &key) const
{
...
// if there are dups - this will return rightmost position
int BTNode::FindKeyPosition(const void* &key) const
{
  assert(IsValid() == 0);
...
RID BTNode::GetAddr(const int pos) const
{
  assert(IsValid() == 0);
  if(pos < 0 || pos > numKeys)
    return RID(-1, -1);
...
RID BTNode::FindAddr(const void* &key) const
{
  assert(IsValid() == 0);
  int pos = FindKey(key);
...
int BTNode::FindKey(const void* &key, const RID& r) const
{
  assert(IsValid() == 0);
...
int BTNode::CmpKey(const void * a, const void * b) const
{
  if (attrType == STRING) {
...
void BTNode::Print()
{
  //std::cerr << GetLeft() << "<--"<< pageRID.Page() << "{";
    std::cerr << "page# "<<pageRID.Page() << ";order "<<order<<";numKeys "<<numKeys<<"{";
